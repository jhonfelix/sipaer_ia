...
const STARTERS = [
  { icon: ClipboardList, color: "text-blue-400", bg: "bg-blue-400/10", title: "Edital de Licitação", description: "Elabore editais de Pregão Eletrônico conforme Lei 14.133/2021", prompt: "Elabore um modelo de edital de licitação na modalidade Pregão Eletrônico para contratação de serviços de TI, conforme a Lei 14.133/2021." },
      ...
      <div className="w-64 flex flex-col border-r border-border bg-sidebar shrink-0">
        ...
        <div className="px-4 py-4 border-b border-border space-y-3">
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar ação..." className="w-full pl-8 pr-3 py-2 rounded-lg bg-muted/40 border border-border text-foreground/70 placeholder:text-muted-foreground/50 text-xs outline-none focus:border-blue-400/30 focus:bg-muted/60 transition-all" />
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <button key={a.id} onClick={() => handleSend(a.prompt)} className="w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-muted/40 transition-colors text-left group">
                <div className="w-7 h-7 rounded-lg bg-blue-500/10 border border-blue-400/15 flex items-center justify-center shrink-0 group-hover:bg-blue-500/20 transition-colors">
                ...
                <ChevronRight className="w-3 h-3 text-muted-foreground/40 group-hover:text-blue-400 shrink-0 transition-colors" />
              </button>
            );
          })}
        </div>
        ...
        <div className="border-t border-border px-3 py-3 space-y-2">
          ...
          {LEGAL_REFS.map(r => (
            <div key={r.label} className="flex items-start gap-2">
              <div className="w-1 h-1 rounded-full bg-blue-400/40 shrink-0 mt-1.5" />
              <div>
                <p className="text-blue-400/80 text-[11px] font-mono font-medium">{r.label}</p>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="px-4 py-3.5 rounded-2xl rounded-tl-sm bg-muted/25 border border-border/60">
                    <span className="flex items-center gap-1.5">
                      {[0, 1, 2].map(i => (
                        <span key={i} className="w-1.5 h-1.5 rounded-full bg-blue-400/60 animate-bounce" style={{ animationDelay: `${i * 160}ms` }} />
                      ))}
                    </span>
                  </div>
                </div>
